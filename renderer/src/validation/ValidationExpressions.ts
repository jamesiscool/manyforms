...
export const useValidationExpressions = () => {
	...
	interface ValidationExpression {
		validate: (path: string, fieldDef: ElementDef, expression: string) => boolean,
		defaultMessage: string
	}

	const validationsExpressions: { [name: string]: ValidationExpression } = {
		validIf: {
			validate: (path: string, fieldDef: ElementDef, expression: string) => evaluate(path, fieldDef, expression),
		...
		requiredIf: {
			validate: (path: string, fieldDef: ElementDef, expression: string) => {
